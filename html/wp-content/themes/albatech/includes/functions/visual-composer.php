...
add_action( 'wp_enqueue_scripts', 'fusion_vc_scripts', 999 );
add_action( 'vc_before_init', 'fusion_map_shortcodes' );

/**
 * Register an action to wrap the image comparison
 * shortcode
 */
add_action( 'wp', 'fusion_image_compare_wrapper' );

if ( ! function_exists( 'fusion_vc_scripts' ) ) {
	/**
	 * Unregister visual composer styles and scripts
	 * 
	 * @return  void
	 */
	function fusion_vc_scripts() {
...
if ( ! function_exists( 'fusion_map_shortcodes' ) ) {
	function fusion_map_shortcodes() {
		if ( shortcode_exists( 'sciba' ) ):

			vc_map( array(
				'base'        => 'sciba',
				'name'        => esc_html__( 'Fusion: Image Comparison', 'hnk' ),
				'icon'        => 'linethemes-shortcode',
				'category'    => esc_html__( 'Fusion', 'hnk' ),
				'params'      => array(
					array(
						'type'       => 'attach_image',
...
if ( ! function_exists( 'fusion_image_compare_wrapper' ) ) {
	function fusion_image_compare_wrapper( $atts, $content = '' ) {
