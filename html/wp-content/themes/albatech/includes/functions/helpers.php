...
if ( ! function_exists( 'fusion_style_switcher_settings' ) ) {
	add_filter( 'style-switcher/settings', 'fusion_style_switcher_settings' );

	function fusion_style_switcher_settings( $settings ) {
		$settings['scheme'] = op_option( 'scheme_color' );
		$settings['colors'] = array(
			'#8ccc50', '#e948ae', '#0088cc', '#f99373'
		);
...
function predefined_background_patterns() {
	...
	if ( empty( $patterns ) || ! is_array( $patterns ) ) {
		...
		if ( $template_directory != $stylesheet_directory && is_dir( $stylesheet_directory . '/assets/img/patterns/' ) ) {
...
if ( ! function_exists( 'fusion_upload_mimes' ) ) {
	add_filter( 'upload_mimes', 'fusion_upload_mimes' );
	 ...
	 * @return  array
	 */
	function fusion_upload_mimes( $mimes ){
...
if ( ! function_exists( 'fusion_under_construction_mode' ) ) {
	add_action( 'wp', 'fusion_under_construction_mode' );
	 ...
	 * @return  void
	 */
	function fusion_under_construction_mode() {
...
if ( is_admin() ) {
	add_action('admin_print_scripts-post-new.php', 'fusion_pricing_table_styles');
	add_action('admin_print_scripts-post.php', 'fusion_pricing_table_styles');

	function fusion_pricing_table_styles() {
		global $post_type;
 ...
 * @return  array
 */
function fusion_sidebars_dropdown_options() {
