...
if ( ! function_exists( 'fusion_head_title' ) ) {
	add_filter( 'wp_title', 'fusion_head_title', 10, 2 );
	 ...
	 * @return  string
	 */
	function fusion_head_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'fusion' ), max( $paged, $page ) );
		}
...
if ( ! function_exists( 'fusion_body_classes' ) ) {
...
if ( ! function_exists( 'fusion_post_classes' ) ) {
	add_filter( 'post_class', 'fusion_post_classes' );

	function fusion_post_classes( $classes ) {
...
if ( ! function_exists( 'fusion_override_page_menu' ) ) {
	add_filter( 'wp_nav_menu_args', 'fusion_override_page_menu' );
	 ...
	 * @return  array
	 */
	function fusion_override_page_menu( $params ) {
		if ( is_page() ) {
...
if ( ! function_exists( 'fusion_extra_menu_items' ) ) {
	add_action( 'theme/after_primary_menu', 'fusion_extra_menu_items' );
	 ...
	 * @return  void
	 */
	function fusion_extra_menu_items( $args ) {
		if ( $args['env'] == 'desktop' ) {
...
if ( ! function_exists( 'fusion_fix_post_title' ) ) {
	add_filter( 'the_title', 'fusion_fix_post_title', 10, 2 );
	 ...
	 * @return  string
	 */
	function fusion_fix_post_title( $title, $id ) {
		$post = get_post( $id );
