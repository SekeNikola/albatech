...
if ( op_option( 'logo_image' ) == true ) {
	...
	$site_name = sprintf( '<img class="logo-standard" src="%s" %s>',
		esc_url( op_option( 'logo_src' ) ),
		op_attributes( $logo_attrs )
	);

	if ( $logo_retina = op_option( 'logo_retina_src' ) ) {
		$site_name.= sprintf( '<img class="logo-retina" src="%s" alt="%s" />', esc_url( $logo_retina ), esc_attr( get_bloginfo( 'name' ) ) );
	}
}
...
// Open .brand
printf( '<div%s>', op_attributes( array( 'id' => 'site-logo', 'class' => $brand_classes ), 'fusion_brand_attrs' ) );
